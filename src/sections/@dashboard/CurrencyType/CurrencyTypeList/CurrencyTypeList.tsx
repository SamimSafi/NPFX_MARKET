...
import MeasurementTableToolbar from './CurrencyTypeTableToolbar';
import CurrencyTypeDelete from './CurrencyTypeDelete';
import { ICurrencyType } from 'src/@types/foamCompanyTypes/looks/CurrencyType';
import CurrencyTypeTableRow from './CurrencyTypeTableRow';

// ----------------------------------------------------------------------

export default observer(function CurrencyTypeList() {
  ...
  const handleFilterName = (filterName: string) => {
    ...
    if (filterName.length > 1) {
      ...
      CurrencyTypeearch({ pageIndex: 0, pageSize: rowsPerPage });
    }
  };

  const handleOpenConfirm = (id: number) => {
    setOpenCloseDialog();
    setCurrencyTypeId(id);
  };
  ...
  const handleEditRow = (id: number) => {
    getCurrencyTypeFromRegistry(id);
    navigate(PATH_DASHBOARD.CurrencyType.edit);
  };
  ...
  const handleChangePage = (event: unknown, newPage: number) => {
    setPage(newPage);
    loadCurrencyType({ pageIndex: newPage, pageSize: rowsPerPage });
  };
  const handlePageSizeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    onChangeRowsPerPage(event);
    let pageZize = parseInt(event.target.value);
    loadCurrencyType({ pageIndex: 0, pageSize: pageZize });
  };
  useEffect(() => {
    if (CurrencyTypeRegistry.size <= 1) {
      loadCurrencyType({ pageIndex: 0, pageSize: rowsPerPage });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const dataFiltered = applySortFilter({
    tableData: CurrencyTypeList,
        ...
        <HeaderBreadcrumbs
          ...
          action={
            <Button
              ...
              to={PATH_DASHBOARD.CurrencyType.new}
            >
        ...
        <Card>
          ...
          <Box sx={{ position: 'relative' }}>
            ...
            <MyDialog
              ...
              size="md"
            >
              <CurrencyTypeDelete id={CurrencyTypeId} />
            </MyDialog>
            <FormControlLabel

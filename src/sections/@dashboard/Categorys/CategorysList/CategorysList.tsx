...
import CategorysDelete from './CategorysDelete';
import { ICategorys } from 'src/@types/foamCompanyTypes/looks/Categorys';
import CategorysTableToolbar from './CategorysTableToolbar';
import CategorysTableRow from './CategorysTableRow';

// ----------------------------------------------------------------------

export default observer(function CategorysList() {
  ...
  const [CategorysId, setCategorysId] = useState<number>(0);
  const TABLE_HEAD = [
  ...
  const handleFilterName = (filterName: string) => {
    ...
    if (filterName.length > 1) {
      ...
      Categorysearch({ pageIndex: 0, pageSize: rowsPerPage });
    }
  };
  ...
  const handleEditRow = (id: number) => {
    getCategorysFromRegistry(id);
    navigate(PATH_DASHBOARD.Categorys.edit);
  };
  ...
  const handleChangePage = (event: unknown, newPage: number) => {
    setPage(newPage);
    loadCategorys({ pageIndex: newPage, pageSize: rowsPerPage });
  };
  const handlePageSizeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    onChangeRowsPerPage(event);
    let pageZize = parseInt(event.target.value);
    loadCategorys({ pageIndex: 0, pageSize: pageZize });
  };
  useEffect(() => {
    if (CategorysRegistry.size <= 1) {
      loadCategorys({ pageIndex: 0, pageSize: rowsPerPage });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const dataFiltered = applySortFilter({
    tableData: CategorysList,
  ...
  return (
    <Page title={translate('Categorys.Title')}>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          heading={translate('Categorys.CategorysList')}
          links={[
          ...
          action={
            <Button
              ...
              to={PATH_DASHBOARD.Categorys.new}
            >
        ...
        <Card>
          <CategorysTableToolbar filterName={filterName} onFilterName={handleFilterName} />
          ...
          <Box sx={{ position: 'relative' }}>
            ...
            <MyDialog
              ...
              size="md"
            >
              <CategorysDelete id={CategorysId} />
            </MyDialog>
            <FormControlLabel
